...
start "Agent Backend" cmd /c "cd /d %ROOT%agent-backend && set PYTHONPATH=%ROOT%llm-layer;%PYTHONPATH% && "%ROOT%.venv\Scripts\python.exe" -m uvicorn app.main:app --host 0.0.0.0 --port 8000 --reload"

timeout /t 2 /nobreak >nul

echo [3/3] Starting Frontend Dev Server (port 3000)...
start "DocPilot Frontend" cmd /c "cd /d %ROOT%frontend && npx http-server . -p 3000 --cors"
